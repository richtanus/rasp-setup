...
function enable_php_apache() {
    install_log "Enabling PHP for apache, since we install phpmyadmin, assume the apache is ready"
...
# Generate logging enable/disable files for hostapd
function create_logging_scripts_bad() {
...
function install_dependencies() {
    ...
    sudo apt-get install apache2 php7.0 php7.0-cgi git hostapd dnsmasq || install_error "Unable to install dependencies"
}
